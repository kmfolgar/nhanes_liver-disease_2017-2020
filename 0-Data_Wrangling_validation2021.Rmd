...
This Rmd file aims to validate the creation of each new variable in the NHANES 2017-2020 dataset.
...
The following code opens the datasets, select the columns and using the nhanesA package finds all the appropriate labels for the categories. 


```{r load_data}
## Demographics 
demo_p <- read_xpt("nhanes_2021_2023/DEMO_L.xpt") %>% 
            select(SEQN, RIDAGEYR, RIAGENDR, RIDRETH3, DMDEDUC2, INDFMPIR, 
                   RIDSTATR, SDMVPSU, SDMVSTRA, WTINT2YR, WTMEC2YR) %>% 
            nhanesTranslate("DEMO_L", colnames = colnames(.), data=.)

saveRDS(demo_p, "nhanes_2021_2023/translated_nhanes/demo_l.rds")


## BMI
bmx_p <- read_xpt("nhanes_2021_2023/BMX_L.XPT") %>% 
          select(SEQN, BMXBMI, BMXWAIST, BMXHIP) #%>% 
          #nhanesTranslate("BMX_J", colnames = colnames(.), data=.)

saveRDS(bmx_p, "nhanes_2021_2023/translated_nhanes/bmx_l.rds")

## Diabetes
diq_p <- read_xpt("nhanes_2021_2023/DIQ_L.XPT") %>% 
          select(SEQN, DIQ010, DIQ050, DIQ070) %>% 
          nhanesTranslate("DIQ_L", colnames = colnames(.), data=.)

saveRDS(diq_p, "nhanes_2021_2023/translated_nhanes/diq_l.rds")

## Glycohemoglobin
ghb_p <- read_xpt("nhanes_2021_2023/GHB_L.XPT") %>% 
          select(SEQN, LBXGH) #%>% 
            #nhanesTranslate("GHB_J", colnames = colnames(.), data=.)

saveRDS(ghb_p, "nhanes_2021_2023/translated_nhanes/ghb_l.rds")

## Alcohol Use Questionnaire
alq_p <-  read_xpt("nhanes_2021_2023/ALQ_L.XPT") %>% 
            select(SEQN, ALQ121, ALQ130, ALQ142, ALQ111, ALQ270) %>% 
            nhanesTranslate("ALQ_L", colnames = colnames(.), data=.)

saveRDS(alq_p, "nhanes_2021_2023/translated_nhanes/alq_l.rds")

## Hepatitis B: Core antibody, Surface antigen, and Hepatitis D antibody
#hep_b_1 <- read_xpt("nhanes_2021_2023/HEPBD_L.XPT") %>% 
#            select(SEQN, LBXHBC, LBDHBG) %>% 
#            nhanesTranslate("HEPBD_L", colnames = colnames(.), data=.)

#saveRDS(hep_b_1, "nhanes_2021_2023/translated_nhanes/hep_b_1.rds")

## Hepatitis B: Surface Antibody
hep_b_2 <- read_xpt("nhanes_2021_2023/HEPB_S_L.XPT") %>% 
            select(SEQN, LBXHBS) %>% 
            nhanesTranslate("HEPB_S_L", colnames = colnames(.), data=.)

saveRDS(hep_b_2, "nhanes_2021_2023/translated_nhanes/hep_b_2.rds")


## Hepatitis C: RNA (HCV-RNA), Confirmed Antibody (INNO-LIA), & Genotype 
#hep_c <- read_xpt("nhanes_2021_2023/HEPC_L.XPT") %>% 
#          select(SEQN, LBDHCI) %>% 
#          nhanesTranslate("HEPC_L", colnames = colnames(.), data=.)

#saveRDS(hep_c, "nhanes_2021_2023/translated_nhanes/hep_c.rds")

## Standard Biochemistry Profile
biopro_p <- read_xpt("nhanes_2021_2023/BIOPRO_L.XPT") %>% 
              select(SEQN, LBXSATSI, LBXSASSI) #%>% 
              #nhanesTranslate("BIOPRO_J", colnames = colnames(.), data=.)

saveRDS(biopro_p, "nhanes_2021_2023/translated_nhanes/biopro_l.rds")

## Liver Ultrasound Transient Elastography
lux_p <- read_xpt("nhanes_2021_2023/LUX_L.XPT") %>%
          select(SEQN, LUXSMED, LUXCAPM, LUAXSTAT) %>% 
          nhanesTranslate("LUX_L", colnames = colnames(.), data=.)

saveRDS(lux_p, "nhanes_2021_2023/translated_nhanes/lux_l.rds")

## Hospital Utilization & Access to Care
huq_p <- read_xpt("nhanes_2021_2023/HUQ_L.XPT") %>% 
          select(SEQN, HUQ030) %>% 
          nhanesTranslate("HUQ_L", colnames = colnames(.), data=.)

saveRDS(huq_p, "nhanes_2021_2023/translated_nhanes/huq_p.rds")

## Health Insurance
hiq_p <- read_xpt("nhanes_2021_2023/HIQ_L.XPT") %>% 
          select(SEQN, HIQ011) %>% 
          nhanesTranslate("HIQ_L", colnames = colnames(.), data=.)

saveRDS(hiq_p, "nhanes_2021_2023/translated_nhanes/hiq_l.rds")


mcq_p <- read_xpt("nhanes_2021_2023/MCQ_L.XPT") %>% 
          select(SEQN,MCQ160L) %>% 
          nhanesTranslate("MCQ_L", colnames = colnames(.), data=.)

saveRDS(mcq_p, "nhanes_2021_2023/translated_nhanes/mcq_l.rds")
...
#  left_join(hep_c, by="SEQN") %>%
  ...
  left_join(lux_p, by="SEQN") %>% 
  mutate_if(is.factor, as.character) -> nhanes

nhanes %>% 
  saveRDS("proccessed_data/data_nhanes_original_2021_2023.rds")
...
```{r load_data}
nhanes_2017_2020 <- readRDS("proccessed_data/data_nhanes_original_2021_2023.rds")
```
...
nhanes_2017_2020 <- nhanes_2017_2020 %>%
  mutate(race = case_when(
    ...
    RIDRETH3 == "Other Race - Including Multi-Racial" ~ "Other Race"
  ))
...
nhanes_2017_2020 <- nhanes_2017_2020 %>%
  mutate(education3 = case_when(
    ...
    DMDEDUC2 == "9-11th grade (Includes 12th grade with no diploma)" ~ "Less than high school",
    DMDEDUC2 == "High school graduate/GED or equivalent" ~ "Completed High school",
...
nhanes_2017_2020 <- nhanes_2017_2020 %>%
  mutate(education = case_when(
    DMDEDUC2 == "Less than 9th grade" ~ "Less than high school",
    DMDEDUC2 == "9-11th grade (Includes 12th grade with no diploma)" ~ "Less than high school",
    DMDEDUC2 == "High school graduate/GED or equivalent" ~ "Completed High school or above",
...
nhanes_2017_2020 %>% 
  tabyl(HUQ030, regular_healthcare)
```
...
nhanes_2017_2020 %>% 
  tabyl(access_healthcare, health_insurance)
```


```{r}
nhanes_2017_2020 %>% 
  tabyl(access_healthcare, regular_healthcare)
```
...
```
```{r}
#nhanes_2017_2020 %>% tabyl(race,HepB_Status)
```
...
nhanes_2017_2020 <- nhanes_2017_2020 %>%
  mutate(complete_case = case_when(
    ...
    (RIDAGEYR >= 20 & RIDSTATR=="Both interviewed and MEC examined") ~ 1))
...
```{r}
nhanes_2017_2020 <- nhanes_2017_2020 %>%
  mutate(complete_case2 = case_when(
    !is.na(age1) &
    !is.na(sex) & 
    !is.na(education) & 
    !is.na(poverty) &
...
tabyl(nhanes_2017_2020$complete_case2)
```
...
```{r}
nhanes_2017_2020 %>% 
  saveRDS("proccessed_data/data_nhanes_processsed_2021.rds")
```
...
nhanes_2017_2020 %>% 
  filter(!is.na(age1) & 
           !is.na(sex) & 
           RIDAGEYR >= 20 & 
           RIDSTATR=="Both interviewed and MEC examined") %>% 
  dim()
```
...
```{r}
nhanes_2017_2020 %>% filter(!is.na(age1) & 
                              !is.na(sex) & 
                              RIDAGEYR >= 20 & 
                              RIDSTATR=="Both interviewed and MEC examined" &
                              !is.na(education) & 
