...
```
```{r}
nhanes_2017_2020 %>% filter(!is.na(age1) & !is.na(sex) & RIDAGEYR >= 20 & RIDSTATR=="Both interviewed and MEC examined") %>% dim()
```

```{r}
nhanes_2017_2020 %>% filter(!is.na(age1) & !is.na(sex) & RIDAGEYR >= 20 & RIDSTATR=="Both interviewed and MEC examined" & !is.na(education) & !is.na(poverty)) %>% dim()
```
